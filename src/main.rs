use std::{collections::VecDeque, error::Error, ffi::OsStr, fs::{FileType, Metadata}, marker::PhantomData, os::windows::fs::FileTypeExt, path::{Path, PathBuf}, str::FromStr, sync::{Arc, Weak}};
use serde::{Deserialize, Serialize};
use serde_json::{self, to_string_pretty};
use tokio::{io::{AsyncWriteExt, stdout}, join, sync::{RwLock, mpsc::{Sender, UnboundedReceiver, UnboundedSender, channel}}};
...
pub async fn amain(){
    ...
    let (_,_) = join!(crawler.crawl(dir.as_path()), writer.run());
}
...
impl Writer{
    pub async fn run(&mut self){
        ...
        async fn print_element(element: &OwnedFsElement, stdout: &mut tokio::io::Stdout, is_first: &mut bool){
            if *is_first{
                *is_first = false;
            } else {
                stdout.write_all(b",\n").await.unwrap();
        ...
        while let Some(weak_element) = self.input.recv().await{
            let Some(element) = weak_element else {break;};
...
impl Crawler{
    pub async fn crawl(&self, dir_path: &Path) -> Result<OwnedFsElement, anyhow::Error> {
        async fn crawl_inner(dir_path: &Path, parent: Option<Weak<FsDir>>, sender: &UnboundedSender<Option<WeakFsElement>>) -> Result<Arc<FsDir>, anyhow::Error> {
            let mut entries = tokio::fs::read_dir(dir_path).await?;

            let dir = Arc::new(FsDir{
                name: dir_path.file_name().unwrap_or(dir_path.as_os_str()).to_string_lossy().into_owned(),
                ...
                let open_entry_task = async {
                    ...
                    let entry = {
                        if entry_type.is_dir(){
                            let dir = Box::pin(crawl_inner(entry.path().as_path(), Some(dir_cell.clone()), sender)).await?;
        ...
        let owned_dir = crawl_inner(dir_path, None, &self.elements_sender).await?;
...
impl Into<FsFileType> for FileType{
    fn into(self) -> FsFileType {
        if self.is_dir(){
            ...
            FsFileType::SymlinkFile
        } else if self.is_symlink_dir() {
            FsFileType::SymlinkDir
        } else {
